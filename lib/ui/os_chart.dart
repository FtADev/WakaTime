...
class OSChartState extends State<OSChart> {
  ...
  buildChart(UserData userData) {
    ...
    for (int i = 0; i < osNames.length; i++) {
      for (int j = 0; j < userData.data.length; j++) {
        for (int k = 0; k < userData.data[j].operatingSystems.length; k++) {
          if (userData.data[j].operatingSystems[k].name == osNames[i]) {
            totalSecond[i] += userData.data[j].operatingSystems[k].totalSeconds;
          }
        }
      }
      sum += totalSecond[i];
    ...
    //Sort
    for (int i = 0; i < osNames.length - 1; i++) {
      for (int j = i + 1; j < osNames.length; j++) {
        if (totalSecond[i] < totalSecond[j]) {
          double tmpS = totalSecond[i];
          totalSecond[i] = totalSecond[j];
          totalSecond[j] = tmpS;

          String tmpN = osNames[i];
          osNames[i] = osNames[j];
          osNames[j] = tmpN;
        }
      }
